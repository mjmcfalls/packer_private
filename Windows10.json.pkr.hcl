...
build {
  ...
  provisioner "file" {
    source      = "./scripts/InstallApplication.ps1"
    destination = "${var.win_temp_dir}\\InstallApplication.ps1"
    direction   =  "upload"
  }
  
  provisioner "file" {
    source      = "./scripts/download_r.ps1"
    destination = "${var.win_temp_dir}\\download_r.ps1"
    direction   =  "upload"
  }

  provisioner "file" {
    source      = "./scripts/Get_r_studio.ps1"
  ...
  provisioner "powershell" {
    inline = ["${var.win_temp_dir}\\download_r.ps1 -uri '${var.r_download_uri}' -OutPath '${var.win_temp_dir}' -install"]
  }
  
  provisioner "powershell" {
    inline = ["${var.win_temp_dir}\\Get_r_studio.ps1 -uri '${var.r_studio_download_uri}' -OutPath '${var.win_temp_dir}' -install"]
  }
